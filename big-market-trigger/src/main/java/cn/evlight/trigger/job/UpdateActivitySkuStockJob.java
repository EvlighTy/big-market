...
public class UpdateActivitySkuStockJob {
    ...
    public void exec() {
        try {
            log.info("[Scheduled Task]-[更新活动sku库存] 开始执行...");
            ActivitySkuStockKeyVO activitySkuStockKeyVO = skuStock.takeQueueValue();
            if (null == activitySkuStockKeyVO){
                log.info("[Scheduled Task]-[更新活动sku库存] 未查询到更新消息");
                return;
            }
            log.info("[Scheduled Task]-[更新活动sku库存] 查询到消息 sku:{} activityId:{}", activitySkuStockKeyVO.getSku(), activitySkuStockKeyVO.getActivityId());
            ...
            if (skuStock.SkuStockIsZero(cacheKey)){
                log.info("[Scheduled Task]-[更新活动sku库存] 库存已清空无需处理此消息");
                return;
            }
            //更新数据库库存
            skuStock.updateActivitySkuStock(sku);
        } catch (Exception e) {
            log.error("[Scheduled Task]-[更新活动sku库存] 执行失败", e);
        }
    }
